...
static cd_t    *cds;        /* implicitly initialised to NULL (0) */
...
static void general_open(char *new_fname)
{
     ...
     */
    new_cds = open_file(new_fname, &new_count);
    if (new_count == -1)        /* an error from open_file() */
    {
...
static void edit_add()
{
     ...
     */
    cds = increase_array(cds, count, 1);
