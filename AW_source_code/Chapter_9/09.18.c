 ...
 */
char *
nums_to_string_date(char       *date_string,     /* output - string 
  representation 	*/
                    int         month,           /* input  - 	*/
                    int         day,             /*   representation	*/
                    int         year,            /*   as three numbers	*/
                    const char *month_names[])   /* input  - string representations of months	*/
{
      sprintf(date_string, "%d %s %d", day, month_names[month - 1], year);
      return (date_string);
}

#define NOT_FOUND -1   /*  Value returned by search function if target 
not found	*/
/*
